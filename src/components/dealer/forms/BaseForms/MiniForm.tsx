...
import { createMiniProtocol, createProProtocol, Mini, Pro } from "../../../../types/product/base";
...
export const MiniForm = ({ setOpen }: { setOpen: React.Dispatch<React.SetStateAction<boolean>> }) => {

    console.log("In Mini form")
    ...
    const {
        ...
        resolver: zodResolver(createMiniProtocol(handrailKey)),
        mode: "onBlur",
        defaultValues: {
            handrail: null
        }
    ...
    useEffect(() => {
        setValue("baseProfileID", "F55");
        setValue("base.anchor.anchorID", "HK")
        setValue("base.anchor.anchorType", "H8")
        setValue("base.anchor.baseProfileID", "F55")
        ...
        <>
            <YStack id="Test" flex={1} style={{ alignItems: "start", justifyContent: "space-between", gap: 16 }}>
                <ScrollView width={"100%"} contentContainerStyle={{
                    ...
                    <YStack style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", justifyContent: "flex-start", gap: 8 }}>
                        <Text style={{ fontSize: 14, fontWeight: "bold" }}>Length quantity</Text>
                        <XStack width={"70%"} style={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                            <Text style={{ fontSize: 14 }}>For 10mm</Text>
                            <Controller
                                control={control}
                                rules={{
                                    maxLength: 100,
                            ...
                            {errors.base?.length?.[0] && (
                                <Text style={{ color: "red", fontSize: 12, }}>{errors.base.length[0].message}</Text>
                            )}
                        </XStack>
                        <XStack width={"70%"} style={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                            <Text style={{ fontSize: 14 }}>For 16mm</Text>
                            <Controller
                                control={control}
                                rules={{
                                    maxLength: 100,
                                ...
                                name="base.length.1"
                            />
                            {errors.base?.length?.[1] && (
                                <Text style={{ color: "red", fontSize: 12, }}>{errors.base.length[1].message}</Text>
                            )}
                        </XStack>
                    </YStack>
